/**
 * \file src/bs_coalform_fixed_users.hpp
 *
...
std::basic_ostream<CharT,CharTraitsT>& operator<<(std::basic_ostream<CharT,CharTraitsT>& os, options<RealT> const& opts)
{
	...
	os << ", output-trace-data-file: " << opts.output_trace_data_file;
...
#ifdef DCS_BS_GT_CI_ON_PARTITION_VALUE_RATIO
	std::vector< boost::shared_ptr< dcs::bs::ci_mean_estimator<RealT> > > ci_rpp_stats(1);
	ci_rpp_stats[0] = boost::make_shared< dcs::bs::ci_mean_estimator<RealT> >(0.95, 0.04);
	ci_rpp_stats[0]->name("RPP");
#endif // DCS_BS_GT_CI_ON_PARTITION_VALUE_RATIO
	std::vector< boost::shared_ptr< dcs::bs::ci_mean_estimator<RealT> > > ci_rp_stats(tot_nb); // Relative profit increments
...
#ifdef DCS_BS_GT_CI_ON_PROFIT_VALUE
	std::vector< boost::shared_ptr< dcs::bs::ci_mean_estimator<RealT> > > ci_ap_stats(tot_nb); // Alone profits
	std::vector< boost::shared_ptr< dcs::bs::ci_mean_estimator<RealT> > > ci_cp_stats(tot_nb); // Coalition profits
#endif // DCS_BS_GT_CI_ON_PROFIT_VALUE

	for (std::size_t b = 0; b < tot_nb; ++b)
	{
		std::ostringstream oss;

		ci_rp_stats[b] = boost::make_shared< dcs::bs::ci_mean_estimator<RealT> >(0.95, 0.04);
		oss.str("");
		oss << "RP_{" << b << "}";
		ci_rp_stats[b]->name(oss.str());

		ci_on_stats[b] = boost::make_shared< dcs::bs::ci_mean_estimator<RealT> >(0.95, std::numeric_limits<RealT>::infinity());
		oss.str("");
		oss << "ON_{" << b << "}";
		ci_on_stats[b]->name(oss.str());

		ci_xl_stats[b] = boost::make_shared< dcs::bs::ci_mean_estimator<RealT> >(0.95, std::numeric_limits<RealT>::infinity());
		oss.str("");
		oss << "XL_{" << b << "}";
		ci_xl_stats[b]->name(oss.str());

#ifdef DCS_BS_GT_CI_ON_PROFIT_VALUE
		ci_ap_stats[b] = boost::make_shared< dcs::bs::ci_mean_estimator<RealT> >(0.95, 0.04);
		oss.str("");
		oss << "AP_{" << b << "}";
		ci_ap_stats[b]->name(oss.str());

		ci_cp_stats[b] = boost::make_shared< dcs::bs::ci_mean_estimator<RealT> >(0.95, 0.04);
		oss.str("");
		oss << "CP_{" << b << "}";
		ci_cp_stats[b]->name(oss.str());
#endif // DCS_BS_GT_CI_ON_PROFIT_VALUE
	}

	// Compute number of users
	const std::size_t num_time_slots = ::std::ceil(opts.time_span/time_step);
	ublas::matrix<std::size_t> prov_time_slot_tot_nus(scen.num_providers, num_time_slots, 0); // Total number of peak users per provider and time slot
	ublas::vector<std::size_t> time_slot_tot_nus(num_time_slots, 0); // Total number of peak users per time slot
	ublas::vector<std::size_t> prov_tot_nus(scen.num_providers, 0); // Total number of peak users per provider over all time span
	std::size_t tot_nu = 0;
	for (std::size_t p = 0; p < scen.num_providers; ++p)
	{
		std::size_t prov_tot_nu = 0; // Total number of users for this provider in this time slot

		for (std::size_t bc = 0; bc < scen.num_bs_types; ++bc)
		{
			std::size_t bs_tot_nu = 0;
			for (std::size_t time_slot = 0; time_slot < num_time_slots; ++time_slot)
			{
				...
				const std::size_t bs_slot_nu = std::ceil(scen.provider_bs_max_num_users[p][bc]*optim_info.fopt);
DCS_DEBUG_TRACE("TIME SLOT: " << time_slot << ", PROV: " << p << " - BS Category: " << bc << " - BS_MAX_NU: " << scen.provider_bs_max_num_users[p][bc] << " - BS_SLOT_NU: " << bs_slot_nu);///XXX

				if (bs_slot_nu > bs_tot_nu)
				{
					bs_tot_nu = bs_slot_nu;
				}
				prov_time_slot_tot_nus(p,time_slot) += bs_slot_nu;
				time_slot_tot_nus(time_slot) += bs_slot_nu;
			}

			prov_tot_nu += bs_tot_nu;
		}

		prov_tot_nus(p) = prov_tot_nu;
		tot_nu += prov_tot_nu;
DCS_DEBUG_TRACE("PROV: " << p << " - TOT_NU: " << prov_tot_nu);//XXX
	}
DCS_DEBUG_TRACE("OVERALL TOT_NU: " << tot_nu);//XXX


	// Build auxiliary data structures for provider-user mapping
	std::map< std::size_t, std::pair<std::size_t, std::size_t> > prov_usr; // Map a provider to a pair of indices in prov_tot_nus: provider => <begin_index,end_index>
	ublas::vector<std::size_t> usr_to_cats(tot_nu, 0); // User to category s.t. if usr_to_cats[i]=k means that the i-th user belongs to category k
	{
		...
		for (std::size_t p = 0; p < scen.num_providers; ++p)
		{
			const std::size_t nu = prov_tot_nus(p);
...
DCS_DEBUG_TRACE("PROVIDER: " << p << " - CLASS: " << uc << " - NU: " << nu << " - N: " << nuc << " (TOT NU: " << tot_nu << " - U_OFFS: " << u_offs << ")");//XXX
	...
	std::size_t iter = 0;
#if defined(DCS_BS_GT_CI_ON_PARTITION_VALUE_RATIO)
	while (!util::check_stats(ci_rpp_stats.begin(), ci_rpp_stats.end()))
#elif defined(DCS_BS_GT_CI_ON_PROFIT_VALUE)
	while (!util::check_stats(ci_ap_stats.begin(), ci_ap_stats.end())
		   || !util::check_stats(ci_cp_stats.begin(), ci_cp_stats.end()))
#else // DCS_BS_GT_CI_ON_PROFIT_VALUE
	...
	//usr_bs_dists(u,b) = 100;//XXX: Just to try...
		...
		// Discretize the whole time interval into time slots
		...
		for (std::size_t time_slot = 0; time_slot < num_time_slots; ++time_slot)
		{
			...
			const std::size_t slot_tot_nu = time_slot_tot_nus(time_slot); // Total number of users in this slot
			...
			std::cout << "- Provider :: Peak Number of Users by Provider in this Time Slot = " << ublas::column(prov_time_slot_tot_nus, time_slot) << std::endl;
			...
			for (std::size_t u = 0; u < slot_tot_nu; ++u)
			{
				if (u > 0)
				{
					std::cout << ",";
			...
			while (subset.has_next())
			{
				...
				// Compute the number of users for this coalition
				{
					...
					for (std::size_t ib = 0; ib < coal_nb; ++ib)
					{
						...
						{
							const std::size_t nu = prov_time_slot_tot_nus(p,time_slot);
	...
	//																						   prov_usr_revenues);
				...
				for (std::size_t p = 0; p < np; ++p)
				{
						 ...
						 ++part_it)
					{
						...
						for (std::size_t i = 0; i < players.size(); ++i)
						{
							...
							const std::size_t onu = prov_tot_nus(bs_to_provs(b)); // number of own users
...
#ifdef DCS_BS_GT_CI_ON_PARTITION_VALUE_RATIO
		RealT alone_rpp = 0; // relative partition profit (coalition case)
		RealT coal_rpp = 0; // relative partition profit (alone case)
		for (std::size_t b = 0; b < tot_nb; ++b)
		{
			alone_rpp += bs_tot_alone_profits(b);
			coal_rpp += bs_tot_coal_profits(b);
		}
		ci_rpp_stats[0]->collect(util::relative_increment(coal_rpp, alone_rpp));
#endif // DCS_BS_GT_CI_ON_PARTITION_VALUE_RATIO
		for (std::size_t b = 0; b < tot_nb; ++b)
		{
			ci_rp_stats[b]->collect(util::relative_increment(bs_tot_coal_profits(b), bs_tot_alone_profits(b)));
		...
		for (std::size_t b = 0; b < tot_nb; ++b)
		{
			ci_ap_stats[b]->collect(bs_tot_alone_profits(b));
			ci_cp_stats[b]->collect(bs_tot_coal_profits(b));
		}
#endif // DCS_BS_GT_CI_ON_PROFIT_VALUE
...
#ifdef DCS_BS_GT_CI_ON_PARTITION_VALUE_RATIO
	std::cout << " * WHOLE PARTITION" << std::endl;
	std::cout << "  - RPP statistics: " << ci_rpp_stats[0]->estimate() << " (s.d. " << ci_rpp_stats[0]->standard_deviation() << ") [" << ci_rpp_stats[0]->lower() << ", " << ci_rpp_stats[0]->upper() << "]" << std::endl;
#endif // DCS_BS_GT_CI_ON_PARTITION_VALUE_RATIO
	for (std::size_t b = 0; b < tot_nb; ++b)
	{
		std::cout << " * BS " << b << std::endl;
#ifdef DCS_BS_GT_CI_ON_PROFIT_VALUE
		std::cout << "  - AP statistics: " << ci_ap_stats[b]->estimate() << " (s.d. " << ci_ap_stats[b]->standard_deviation() << ") [" << ci_ap_stats[b]->lower() << ", " << ci_ap_stats[b]->upper() << "]" << std::endl;
		std::cout << "  - CP statistics: " << ci_cp_stats[b]->estimate() << " (s.d. " << ci_cp_stats[b]->standard_deviation() << ") [" << ci_cp_stats[b]->lower() << ", " << ci_cp_stats[b]->upper() << "]" << std::endl;
#endif // DCS_BS_GT_CI_ON_PROFIT_VALUE
		std::cout << "  - RP statistics: " << ci_rp_stats[b]->estimate() << " (s.d. " << ci_rp_stats[b]->standard_deviation() << ") [" << ci_rp_stats[b]->lower() << ", " << ci_rp_stats[b]->upper() << "]" << std::endl;
		std::cout << "  - ON statistics: " << ci_on_stats[b]->estimate() << " (s.d. " << ci_on_stats[b]->standard_deviation() << ") [" << ci_on_stats[b]->lower() << ", " << ci_on_stats[b]->upper() << "]" << std::endl;
		std::cout << "  - XL statistics: " << ci_xl_stats[b]->estimate() << " (s.d. " << ci_xl_stats[b]->standard_deviation() << ") [" << ci_xl_stats[b]->lower() << ", " << ci_xl_stats[b]->upper() << "]" << std::endl;
	}
}
