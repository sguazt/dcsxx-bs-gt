 ...
 *
 * \brief Formation of Nash-stable coalitions of BSs.
 *
...
#ifndef DCS_BS_COALITION_FORMATION_NASH_STABLE_HPP
#define DCS_BS_COALITION_FORMATION_NASH_STABLE_HPP
...
struct nash_stable_coalition_analyzer
{
											   ...
											   partition_info<RealT> const* p_last_partition) const
	{
		...
		while (partition.has_next())
		{
			...
			// For all players $p$
			::std::vector<gtpack::player_type> players(game.players());
			for (std::size_t p = 0; p < game.num_players() && nash_stable; ++p)
			{
					 ...
					 ++sub_it)
				{
					...
					if (coal_players.count(pid) == 0)
					{
						...
						DCS_DEBUG_TRACE("--- AUGMENTED COALITION: " << game.coalition(cid) << " (CID=" << cid << ")");//XXX

						// Check player's preference
						if (::dcs::math::float_traits<RealT>::definitely_greater(visited_coalitions.at(cid).payoffs.at(pid), candidate_partition.payoffs.at(pid)))
						{
							nash_stable = false;
							break;
						}
...
}; // nash_stable_coalition_analyzer
*/
...
#endif // DCS_BS_COALITION_FORMATION_NASH_STABLE_HPP
